...
# Capture frames from the camera and perform processing
def process_frames(imageNumber):
    ...
    while True:
        ...
        if success:  # Check if the frame was successfully captured
            ret, buffer = cv2.imencode('.jpg', img)
            if ret:  # Check if the encoding was successful
...
def gen():
    for frame in process_frames(imageNumber):
        yield (b'--frame\r\n'
